class Spline {
  ...
  // ###### save the heights ###### // 
  float[] facetHeights = new float[0]; // if setting the heights then this will match the facet count
  boolean useUpHeight = true; // whether or not to reverse the height  
  ...
  Spline() {
    ...
    allSplines.add(this);
  } // end default construcot

  //

  Spline getCopy() {
    /*
    Spline cp = new Spline();
     for (int i = 1; i < curvePoints.size() - 1; i++) cp.addCurvePoint(curvePoints.get(i).get());
     cp.makeFacetPoints(minAngleInDegrees, minDistance, divisionAmount, flipUp);
     return cp;
     */
    return new Spline(getJSON());
  } // end getCopy
  ...
  Spline(JSONObject json) {
    ...
    if (json.hasKey("facetPoints")) {
      facetPoints = new PVector[0];
      jar = json.getJSONArray("facetPoints");
      for (int i = 0; i < jar.size(); i++) {
        pt = jar.getJSONObject(i);
        facetPoints = (PVector[])append(facetPoints, new PVector(pt.getFloat("x"), pt.getFloat("y")));
      }
    }
    if (json.hasKey("facetUps")) {
      facetUps = new PVector[0];
      jar = json.getJSONArray("facetUps");
      for (int i = 0; i < jar.size(); i++) {
        pt = jar.getJSONObject(i);
        facetUps = (PVector[])append(facetUps, new PVector(pt.getFloat("x"), pt.getFloat("y")));
      }
    }
    if (json.hasKey("facetRights")) {
      facetRights = new PVector[0];
      jar = json.getJSONArray("facetRights");
      for (int i = 0; i < jar.size(); i++) {
        pt = jar.getJSONObject(i);
        facetRights = (PVector[])append(facetRights, new PVector(pt.getFloat("x"), pt.getFloat("y")));
      }
    }
    if (json.hasKey("distances")) {
      distances = new float[0];
      jar = json.getJSONArray("distances");
      for (int i = 0; i < jar.size(); i++) {
        distances = (float[])append(distances, jar.getFloat(i));
      }
    }
    if (json.hasKey("runningDistances")) {
      runningDistances = new float[0];
      jar = json.getJSONArray("runningDistances");
      for (int i = 0; i < jar.size(); i++) {
        runningDistances = (float[])append(runningDistances, jar.getFloat(i));
      }
    }
    if (json.hasKey("totalDistance")) {
      totalDistance = json.getFloat("totalDistance");
    }
    if (json.hasKey("facetHeights")) {
      facetHeights = new float[0];
      jar = json.getJSONArray("facetHeights");
      for (int i = 0; i < jar.size(); i++) {
        facetHeights = (float[])append(facetHeights, jar.getFloat(i));
      }
    }
    if (json.hasKey("useUpHeight")) {
      useUpHeight = json.getBoolean("useUpHeight");
    }
    if (json.hasKey("flipUp")) {
      flipUp = json.getBoolean("flipUp");
    }
    if (json.hasKey("facetsMade")) {
      facetsMade = json.getBoolean("facetsMade");
    }
    if (json.hasKey("minAngleInDegrees")) {
      minAngleInDegrees = json.getFloat("minAngleInDegrees");
    }
    if (json.hasKey("minDistance")) {
      minDistance = json.getFloat("minDistance");
    }
    if (json.hasKey("divisionAmount")) {
      minDistance = json.getInt("divisionAmount");
    }
    if (json.hasKey("id")) id = json.getInt("id");
  ...
  ArrayList<PVector> getPointAlongSpline(float percentIn) {
    ...
    newPoint.add(new PVector()); // 3 will be the interpolated height point, if any
    ...
    for (int i = 0; i < runningDistances.length - 1; i++) {
      ...
      if (targetDistance == low) {
        ...
        if (facetHeights.length == facetPoints.length) {
          PVector dir = facetUps[i].get();
          dir.mult(facetHeights[i]);
          dir.add(facetPoints[i]);
          newPoint.add(dir);
        }
        else newPoint.add(new PVector());
      }
        ...
        if (facetHeights.length == facetPoints.length) {
          PVector dir = facetUps[i + 1].get();
          dir.mult(facetHeights[i + 1]);
          dir.add(facetPoints[i + 1]);
          newPoint.add(dir);
        }
        else newPoint.add(new PVector());
        break;
        ...
        if (facetHeights.length == facetPoints.length) {
          float dirA = facetHeights[i];
          float dirB = facetHeights[i + 1];
          float dirAvg = (dirA * percentLow + dirB * percentHigh);
          PVector dir = newPoint.get(1).get();
          dir.mult(dirAvg);
          dir.add(newPoint.get(0));
          newPoint.add(dir);
        }
        else newPoint.add(new PVector());
  ...
  void displayHeights() {
    if (facetHeights.length == facetPoints.length) {
      for (int i = 0; i < facetHeights.length; i++) {
        if (facetHeights[i] >= 0) {
          PVector dir = facetUps[i].get();
          dir.mult((useUpHeight ? 1 : -1) * facetHeights[i]);
          line(facetPoints[i].x, facetPoints[i].y, facetPoints[i].x + dir.x, facetPoints[i].y + dir.y);
        }
      }
    }
    else return;
  ...
  // 
  void multiplyFromPoint(PVector pt, float amt) {
    ArrayList<PVector> intersect = getPointByClosestPoint(pt);
    if (intersect == null) {
      println("problem in multiply");
    }
    PVector dir = new PVector();
    pt = intersect.get(0);
    for (int i = 0; i < facetPoints.length; i++) {
      dir = PVector.sub(facetPoints[i], pt);
      dir.mult(amt);
      facetPoints[i] = PVector.add(pt, dir);
      if (i <= facetHeights.length) facetHeights[i] *= amt;
    }
  } // end multiplyFromPoint

  //
  JSONObject getJSON() {
    JSONObject json = new JSONObject();
    JSONArray jar = new JSONArray();
    for (int i = 0; i < curvePoints.size(); i++) jar.setJSONObject(i, getPVectorJSON(curvePoints.get(i)));
    json.setJSONArray("curvePoints", jar);
    jar = new JSONArray();
    for (int i = 0; i < facetPoints.length; i++) jar.setJSONObject(i, getPVectorJSON(facetPoints[i]));
    json.setJSONArray("facetPoints", jar);
    jar = new JSONArray();
    ...
    jar = new JSONArray();
    for (int i = 0; i < runningDistances.length; i++) jar.setFloat(i, runningDistances[i]);
    json.setJSONArray("runningDistances", jar);

    json.setFloat("totalDistance", totalDistance);
    json.setFloat("minAngleInDegrees", minAngleInDegrees);
    json.setFloat("minDistance", minDistance);
    json.setInt("divisionAmount", divisionAmount);
    json.setBoolean("flipUp", flipUp);
    json.setBoolean("facetsMade", facetsMade);
    json.setBoolean("useUpHeight", useUpHeight);

    json.setInt("id", id);

    jar = new JSONArray();
    for (int i = 0; i < facetHeights.length; i++) jar.setFloat(i, facetHeights[i]);
    json.setJSONArray("facetHeights", jar);
