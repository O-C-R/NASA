...
void setup() {
  ...
  for (int year : years) {
    ...
    for (int i = 0; i < allLines.length; i++) {
      ...
      String[] broken = splitTokens(allLines[i].replace(TAB, ' '), " ");
      String tempLine = join(broken, " ");

      broken = splitTokens(tempLine, " ");

      boolean madeNewDay = false;
      boolean monthYearLine = false;
      if (broken.length > 1) {
        String monthCheck = broken[1].trim().replace(":", "");
        String dayCheck = cleanOddCharsOut(broken[0].trim());
