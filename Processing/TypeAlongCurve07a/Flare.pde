class Flare {
  ...
  void makeFlareSpline(Spline s) {
    ...
    int index = 0;
    float flareSplineColorPercent = 0f;
    ...
    for (int i = 0; i < flareLayers; i++) {
      ...
      if (randomValue < oldTotal) {
        index = i;
        float powNumber = .2;
        //flareSplineColorPercent = map(pow(randomValue, powNumber), 0, pow(totalOptions, powNumber), fullGrayColorPercent, lowestGrayColorPercent);
        //flareSplineColorPercent = random(lowestGrayColorPercent, fullGrayColorPercent);
        flareSplineColorPercent = map(pow(randomValue, powNumber), pow(totalOptions, powNumber), 0, lowestGrayColorPercent, fullGrayColorPercent);
        powNumber = .3;
    ...
    FlareSpline fs = new FlareSpline(s, flareSplineColorPercent);
...
class FlareSpline {
  Spline spline = null;
  float splineColorPercent = 0f;

  ArrayList<Label> labels = new ArrayList<Label>();

  //
  FlareSpline(Spline spline, float splineColorPercent) {
    this.spline = spline;
    this.splineColorPercent = constrain(splineColorPercent, lowestGrayColorPercent, fullGrayColorPercent);
    //println("new Spline with percent: " + splineColorPercent); 
  } // end constructor
  ...
  void display() {
    for (Label l : labels) {
      l.fillAlpha = 1f;  
      l.term.fillAlphaPercent = 1f;
      color c = lerpColor(bgColor, flareColor, splineColorPercent);
