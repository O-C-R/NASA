...
void makeAlphaValuesForTerms() {
  ...
  for (int i = 0; i < bucketsAL.size(); i++) {
    ...
    for (int j = 0; j <  thisBucket.bucketTermsPersonsAL.size(); j++) {
      Term t = thisBucket.bucketTermsPersonsAL.get(j);
      makeTermAlpha(t, (float)j / thisBucket.bucketTermsPersonsAL.size(), bucketsAL.get(i).name);
    }
